...
const loadvarient = async(req,res)=>{
   ...
   const varients=await variant.aggregate([{$match:{product:new ObjectId(productId)}},
...
const updatevariant = async (req, res) => {
    try {
        const { productName, editVariant, editVariantPrice, quantity, variantId } = req.body;

        if (!productName || !editVariant || !editVariantPrice || !quantity || !variantId) {
        ...
        res.json({ success: true, message: "Variant updated successfully.", updatedVariant });
